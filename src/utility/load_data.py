...
class Data(object):
    def __init__(self, path, batch_size):
        ...
        unqiue_words = path + '/doc_word_list_squeezed.txt'

        #get number of users and items
        self.n_docs, self.n_qrls, self.n_words = 0, 0, 0
        self.n_train, self.n_test, self.n_valid = 0, 0, 0
        self.pos_pools = {}
        self.neg_pools = {}
        ...
        with open(valid_file) as f_valid: 
            for l in f_valid.readlines():
                ...
                qrl = self.qrl_dict[qrl_key]
                doc = self.doc_dict[doc_key]
                if qrl in self.valid_set:
                    self.valid_set[qrl].append(doc)
                else:
                    self.valid_set[qrl] = [doc]
                self.n_valid += 1
        print('done')
